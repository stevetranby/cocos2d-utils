...
set elements [list "@interface" "class" "id" "bool" "self" "this" "super" "this" "nil" "NULL" "YES" "true" "NO" "false" "#import" "#include" "NSString" "CCString" "NSLog" "CCLOG" "CGSize" "CCSize" "NSString" "CCString" "NSUInteger" "CCInteger" "unichar" "char" "NSMakeRange" "CCRangeMake"]
...
# line passed by value
	global elements
	global delimeters
	set td "\[[lindex $delimeters 0]\]"
	for {set i 1} {$i<[llength $delimeters]} {incr i} {
		append td "|\[[lindex $delimeters $i]\]"
	}
	...
	for {set i 0} {$i<[llength $elements]} {incr i 2} {
		...
		set templ "[lindex $elements $i]($td)"
...
proc handle_super_position { line } {
	...
	#FROM: CCLabelTTF* name = \[CCLabelTTF labelWithString:\[NSString stringWithFormat:@\"%s %s\", actionMethods\[currentActionX\], actionMethods\[currentActionY\]\] fontName:@\"Arial\" fontSize:14];
	#TO: CCLabelTTF* name = CCLabelTTF::labelWithString(CCString::stringWithFormat("%s %s",actionMethods[currentActionX],actionMethods[currentActionY]),"Arial",14);
	#puts "line:$line"
