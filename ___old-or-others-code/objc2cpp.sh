...
then
    ...
    sed -E 's/@property \([a-zA-Z, ]+\)[ ]*([a-zA-Z]+)[ ]+([a-zA-Z]+)/CC_SYNTHESIZE(\1, \2, \2)/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/@property \([a-zA-Z, ]+\)[ ]*([a-zA-Z]+)[* ]+([a-zA-Z]+)/CC_SYNTHESIZE_RETAIN(\1*, \2, \2)/g' $f > "$f.tmp"; cp "$f.tmp" $f
...
# CGRect => cocos2d::CCRect
    ...
    sed -E 's/CGPointMake/CCPointMake/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/CGSizeMake/CCSizeMake/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/CGRectMake/CCRectMake/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/CGPoint/CCPoint/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/CGSize/CCSize/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/CGRect/CCRect/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/CGFloat/float/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/ccTime/float/g' $f > "$f.tmp"; cp "$f.tmp" $f

# NSMutableArray => cocos2d::CCArray
# NSMutableDictionary => cocos2d::CCDictionary
# NSString => cocos2d::CCString

    sed -E 's/NSArray/CCArray/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/NSMutableArray/CCArray/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/NSDictionary/CCDictionary/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/NSMutableDictionary/CCDictionary/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/NSString/CCString/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/NSString/CCString/g' $f > "$f.tmp"; cp "$f.tmp" $f
...
# @end
    ...
    sed -E 's/@class/class/g' $f > "$f.tmp"; cp "$f.tmp" $f
    sed -E 's/@end//g' $f > "$f.tmp"; cp "$f.tmp" $f
